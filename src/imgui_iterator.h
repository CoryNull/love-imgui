...
CALL_FUNCTION_NO_RET(DestroyContext, ctx)
END_IMGUI_FUNC
...
POP_END_STACK(9)
END_IMGUI_FUNC
//    IMGUI_API void          SetTooltip(const char* fmt, ...);                     // set a text-only tooltip, typically use with ImGui::IsItemHovered(). override any previous call to SetTooltip().
// Variadic functions aren't suppported but here it is anyway
...
DEFAULT_ARG(ImGuiMouseButton, mouse_button, 1)
...
LABEL_ARG(str_id)
DEFAULT_ARG(ImGuiTabBarFlags, flags, 0)
...
LABEL_ARG(label)
OPTIONAL_BOOL_POINTER_ARG(p_open)
DEFAULT_ARG(ImGuiTabItemFlags, flags, 0)
...
DEFAULT_ARG(ImGuiMouseButton, mouse_button, 0)
...
DEFAULT_ARG(ImGuiMouseButton, button, 0)
...
CALL_FUNCTION(SaveIniSettingsToMemory, const char*, out_ini_size)
PUSH_STRING(ret)
PUSH_LAST_STRING(ret)
END_IMGUI_FUNC
...
END_STACK_OPTION(9, EndTooltip)
END_STACK_OPTION(10, EndPopup)
END_STACK_OPTION(11, EndTabBar)
END_STACK_OPTION(12, EndTabItem)
END_STACK_OPTION(13, EndDragDropSource)
END_STACK_OPTION(14, EndDragDropTarget)
END_STACK_OPTION(15, EndChildFrame)
END_STACK_END
