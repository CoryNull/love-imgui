...
const char *getRealDirectoryIfExists(lua_State *L, const char *relativePath) {
	...
	if (!lua_isnil(L, -1)) {
		size_t size = 0;
		auto tmp    = lua_tolstring(L, -1, &size);
...
static int w_KeyPressed(lua_State *L) {
	size_t size;
	auto key = luaL_checklstring(L, 1, &size);
	KeyPressed(key);
	return 0;
}
...
static int w_TextInput(lua_State *L) {
	size_t size;
	auto text = luaL_checklstring(L, 1, &size);
	TextInput(text);
	return 0;
}
...
static int w_SetStyleColorU32(lua_State *L) {
	unsigned int name  = luaL_checknumber(L, 1);
	unsigned int value = luaL_checknumber(L, 2);
	SetStyleColorU32(name, value);
	return 0;
}

static int w_SetStyleColorV4(lua_State *L) {
	unsigned int name = luaL_checknumber(L, 1);
	float col[]       = {
	...
	SetStyleColorV4(name, col);
	return 0;
}

static int w_SetStyleValue(lua_State *L) {
	auto name = luaL_checkstring(L, 1);
	auto x    = luaL_checknumber(L, 2);
	auto y    = luaL_optnumber(L, 3, 0.0);
	try {
		SetStyleValue(name, x, y);
	} catch (std::string excuse) {
		// Style name didn't exist...
		lua_pushstring(L, excuse.c_str());
		lua_error(L);
	}
	return 0;
}
...
static int w_SetGlobalFontFromFileTTF(lua_State *L) {
	size_t size;
	auto path          = luaL_checklstring(L, 1, &size);
	...
	auto basePath = getRealDirectoryIfExists(L, path);
	if (basePath == nullptr) {
	...
	SetGlobalFontFromFileTTF(&(fullPath[0]), size_pixels, spacing_x, spacing_y, oversample_x, oversample_y);
	lua_settop(L, 0);
	return 0;
}

static int w_UseGamepad(lua_State *L) {
	auto index = ( unsigned int )luaL_checknumber(L, 1);
	UseGamepad(index);
	return 0;
}
...
static int w_AddFontFromFileTTF(lua_State *L) {
	size_t filenameSize;
	auto filename   = luaL_checklstring(L, 1, &filenameSize);
	float pixelSize = luaL_checknumber(L, 2);

	auto basePath = getRealDirectoryIfExists(L, filename);
	if (basePath == nullptr) {
		lua_pushstring(L, "File does not exist.");
		lua_error(L);
		return 0;
...
#define LABEL_ARG(name)   \
	...
	auto name = luaL_checklstring(L, arg++, &(i_##name##_size));

#define LABEL_POINTER_ARG(name)                                      \
	size_t i_##name##_size;                                            \
	auto content    = luaL_checklstring(L, arg++, &(i_##name##_size)); \
	size_t buf_size = luaL_checknumber(L, arg++);                      \
	auto name       = new char[buf_size];                              \
...
#define OPTIONAL_NUMBER_ARG(name, otherwise) \
	...
	if (arg <= max_args) { name = luaL_checknumber(L, arg++); }
...
#define OPTIONAL_INT_ARG(name, otherwise) \
	int name = otherwise;                   \
	if (arg <= max_args) { name = ( int )luaL_checkinteger(L, arg++); }

#define INT_ARG(name) const int name = ( int )luaL_checkinteger(L, arg++);
...
#define OPTIONAL_UINT_ARG(name, otherwise) \
	...
	if (arg <= max_args) { name = ( unsigned int )luaL_checkint(L, arg++); }
...
#define OPTIONAL_BOOL_ARG(name, otherwise) \
	bool name = otherwise;                   \
	if (arg <= max_args) { name = ( bool )lua_toboolean(L, arg++); }

#define DEFAULT_ARG(type, name, value) type name = value;
...
#include "imgui_iterator.h"
		...
		{"SetStyleColorU32", w_SetStyleColorU32},
		{"SetStyleColorV4", w_SetStyleColorV4},
		{"SetStyleValue", w_SetStyleValue},
...
extern "C" LOVE_IMGUI_EXPORT int luaopen_imgui(lua_State *L) {
	...
	// ImGuiWindowFlags, prefer using the ImGuiXXX_ version instead of the short version as it's deprecated and will be
	// removed
